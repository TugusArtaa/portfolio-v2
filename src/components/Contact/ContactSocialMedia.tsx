...
interface Props {
  contacts: ContactAbout[];
}

export default function ContactSocialMedia({ contacts }: Props) {
  ...
  const socialLinks = [
    ...
    {
      id: "discord",
      href: "https://discord.gg/sgUNzesY",
      icon: (
        <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    <div data-contact-social className="relative z-20 mt-12 sm:mt-16">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <div className="w-12 sm:w-16 h-0.5 bg-gradient-to-r from-sky-500 to-sky-400 rounded-full mx-auto"></div>
      </div>
      <div className="flex flex-wrap gap-3 sm:gap-4 justify-center max-w-2xl mx-auto">
        {/* Render semua social kecuali discord */}
        {socialLinks
          .filter((s) => s.show && s.id !== "discord")
          .map((s) => (
            <a
              key={s.id}
              href={s.href}
              ...
              className={`group flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-sky-200/30 dark:border-sky-400/20 shadow-md shadow-sky-400/5 dark:shadow-sky-500/10 transition-all duration-300 hover:shadow-lg hover:scale-110 text-gray-600 dark:text-gray-300 ${s.color}`}
              aria-label={s.id}
            >
              {s.icon}
            </a>
        ...
        {/* Discord */}
        <div className="w-full flex justify-center sm:w-auto sm:inline-flex">
          {socialLinks
            .filter((s) => s.show && s.id === "discord")
            .map((s) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 rounded-xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-sky-200/30 dark:border-sky-400/20 shadow-md shadow-sky-400/5 dark:shadow-sky-500/10 hover:shadow-lg transition-all duration-300 mt-2 sm:mt-0 cursor-pointer hover:scale-105"
                aria-label="Join Discord Server"
              >
                <span className="text-gray-600 dark:text-gray-300 group-hover:text-indigo-500 transition-colors duration-300">
                  {s.icon}
                </span>
                <span className="text-slate-700 dark:text-slate-200 font-medium text-xs sm:text-sm group-hover:text-indigo-500 transition-colors duration-300">
                  {s.label}
                </span>
              </a>
            ))}
